...
bool validName(const std:: string& name)
{
	...
	while (it != name.end() && !std::isdigit(*it) && !std::isspace(*it))
	{
		it++;
	}
	if (std::isspace(*it))
		std::cout << "I need a single name." << std::endl;
...
int addContact(PhoneBook& phoneBook)
{
	...
	std::cout << "First name:\n";
	if (!(std::getline(std::cin, firstName)) && !handleWeirdInput())
		return (0);
	...
	std::cout << "Last name:\n";
	if (!(std::getline(std::cin, lastName)) && !handleWeirdInput())
		return (0);
	...
	std::cout << "Nickname:\n";
	if (!(std::getline(std::cin, nickname)) && !handleWeirdInput())
		return (0);
	std::cout << std::endl;

	std::cout << "Phone number:\n";
	if (!(std::getline(std::cin, phoneNumber)) && !handleWeirdInput())
		return (0);
	...
	if (!(std::getline(std::cin, darkestSecret)) && !handleWeirdInput())
		return (0);
...
int main()
{
	...
	{
		showOptions();
		if (!(std::getline(std::cin, input)) && !handleWeirdInput())
			return (1);
		if (input == "ADD")
		{
